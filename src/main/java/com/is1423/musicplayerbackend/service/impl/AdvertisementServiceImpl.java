...
public class AdvertisementServiceImpl implements AdvertisementService {
    ...
    public SongResponseDTO getSongByAdvertisement(Long advertisementId) {
        Advertisement advertisement = repository.findById(advertisementId)
            .orElseThrow(() -> new ResourceNotFoundException(EntityNameConstant.ADVERTISEMENT, MessageKeyConstant.NOT_FOUND, advertisementId.toString()));
        Song song = songRepository.findById(advertisement.getSongId())
            .orElseThrow(() -> new ResourceNotFoundException(EntityNameConstant.SONG,  MessageKeyConstant.NOT_FOUND, advertisement.getSongId().toString()));
